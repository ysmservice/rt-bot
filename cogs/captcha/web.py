...
class WebCaptcha:
    def __init__(self, cog: "Captcha", password_length: int = 5):
        ...
        self.view.add_item(
            discord.ui.Button(
                emoji="https://www.google.com/s2/favicons?domain=hcaptcha.com",
                label="Go to webpage"
            )
        )
    ...
    async def on_captcha(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            {"ja": "以下のボタンを押してウェブにある認証ページにてコードを取得してください。\nそしたら取得したコードと同じコードを選んでください。",
             "en": "Click on the button below to get the code on the authentication page on the web.\nThen select the same code as the one you got."},
            view=WebCaptchaView(self, timeout=360)
        )
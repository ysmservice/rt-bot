...
class OldFreeChannel(commands.Cog):
    ...
    @commands.group(
        extras={
            "headding": {
                "ja": "**非推奨** 古いフリーチャンネル機能です。",
                "en": "**DEPRECATED** Make the executed channel a dedicated channel for free channel creation."
            },
            "parent": "Other"
        },
        name="oldfreechannel"
    )
    async def oldfreechannel(self, ctx):
        """!lang ja
        --------
        フリーチャンネルのコマンドです。
    ...
    @oldfreechannel.command(aliases=["add", "rg"])
...
The name of the created voice channel will include the ID of the person who created it, but this ID does not need to be included in the channel name when removing it."""
    ...
    @oldfreechannel.command(name="remove")
    @commands.cooldown(1, 300, commands.BucketType.user)
    @commands.has_permissions(manage_channels=True)
    @setting.Setting(
...
def setup(bot):
    bot.add_cog(OldFreeChannel(bot))